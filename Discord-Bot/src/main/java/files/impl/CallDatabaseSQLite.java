...
public class CallDatabaseSQLite implements CallDatabase {
    ...
    public boolean checkName(long guildId, String name) {
        ...
        try {
            ...
            PreparedStatement pstmt = connection.prepareStatement("SELECT * FROM calldata WHERE guildid = ? AND name = ?");
    ...
    public long saveCallData(Guild guild, String users, long startTime, long endTime, String name, long requester) {
        try {
            ...
            PreparedStatement pstmt = connection.prepareStatement("INSERT INTO calldata(guildid, userids, starttime, endtime, name, requester) VALUES(?,?,?,?,?,?)");
            ...
            ResultSet result = statement.executeQuery("SELECT id FROM calldata WHERE guildid = " + guild.getIdLong() + " AND starttime = " + startTime);
    ...
    public String getUsersFromDb(String search, TextChannel channel, Member member) {
        ...
        try {
            ...
            PreparedStatement prepStmt = connection.prepareStatement("SELECT userids, requester FROM calldata WHERE name = ? AND guildid = ?");
            try {
                Long.parseLong(search);
                //search contains an id
                isId = true;

                result = stmt.executeQuery("SELECT userids, requester FROM calldata WHERE id = " + search + " AND guildid = " + channel.getGuild().getId());
    ...
    public boolean removeGuildData(long guildId) {
        ...
        try {
            ...
            ResultSet result = stmt.executeQuery("SELECT id FROM calldata WHERE guildid = " + guildId);
