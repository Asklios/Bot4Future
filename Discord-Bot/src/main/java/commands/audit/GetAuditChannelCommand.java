        ...
        if (member.hasPermission(channel, Permission.ADMINISTRATOR)) {
            ...
            try {
                ...
                if (auditChannelId != 0) {
                    ...
                    channel.sendMessage("<#" + auditChannelId + "> ist der aktuelle Audit-Channel.").queue(m -> m.delete().queueAfter(10,TimeUnit.SECONDS));
                } else {

                    channel.sendMessage("Der Auditchannel ist noch nicht festgelegt.\n```%audit #channel```").queue(m -> m.delete().queueAfter(10,TimeUnit.SECONDS));
                }
