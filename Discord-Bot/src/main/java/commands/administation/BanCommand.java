		...
		if (banMember != null){
			...
			if (banMember.getRoles().isEmpty() || highestBotRole.canInteract(banMember.getRoles().get(0))) {
				...
				try {
					...
					/*if ( message.getGuild().getId().contentEquals("626752177330716694")) { // wenn es sich bei dem Server um den FFF.de Server handelt wird auf den Entbannungsantrag verwiesen
						pn.setDescription("**Begründung:** " + reason + " \n \n Wenn du Einspruch einlegen möchtest, dann stelle hier deinen Antrag auf Entbannung: <https://pibot.eu/index.php/entbannungsantrag-discord-fff-germany/>");
					}
					else {
						pn.setDescription("**Begründung:** " + reason);
					}*/
